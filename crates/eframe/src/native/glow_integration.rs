                        ...
                        render_state_recovery_diagnostic.as_mut()
                {
                    ...
                    if now.duration_since(*last_logged_at) >= Duration::from_secs(10) {
                        *last_logged_at = now;
                        ...
                        let wgl_context_is_current = context.is_current();
                        log::info!(
                            "post-F7 root swap diagnostic: recovery_elapsed={:.1}s window_id={window_id:?} visible={is_visible} occluded={:?} size={}x{} current_context={} not_current_context={} standby_context={} wgl_context_is_current={wgl_context_is_current}",
                            now.duration_since(*recovery_started_at).as_secs_f32(),
                            viewport.info.occluded,
                            window_size[0],
                            window_size[1],
                            current_gl_context.is_some(),
                            not_current_gl_context.is_some(),
                            standby_gl_context.is_some(),
                        );
                    }
                }
            }
