...
impl GlowWinitRunning<'_> {
    ...
    fn recreate_render_state_with_existing_display(
        ...
        let preparation = (|| -> Result<_> {
            ...
            let replacement_context = match replacement_context.make_current_preserving(&gl_surface)
            {
                Ok(context) => context,
                Err((standby_context, err)) => {
                    glutin.standby_gl_context = Some(standby_context);
                    return Err(err.into());
                }
            };
            #[cfg(target_os = "windows")]
        ...
        let replacement_painter = match egui_glow::Painter::new(
            ...
            Err(err) => {
                // The painter failed before commit, so return the consumed standby to its slot.
                match replacement_context.make_not_current_preserving() {
                    Ok(standby_context) => {
                        glutin.standby_gl_context = Some(standby_context);
                    }
                    Err((_context, release_err)) => {
                        log::warn!(
        ...
        let replacement_context = match replacement_context.make_not_current_preserving() {
            Ok(context) => context,
            Err((context, err)) => {
                let old_surface = glutin
                    .viewport(ViewportId::ROOT)
                    ...
                    .expect("F7 recovery must retain the root GL surface");
                old_context.make_current(old_surface)?;
                // The old context now displaced this one, so it is safe to retain for retry.
                glutin.standby_gl_context =
                    Some(unsafe { context.into_not_current_after_displacement() });
                return Err(err.into());
            }
        };
...
impl GlowWinitRunning<'_> {
    fn run_ui_and_paint(
        ...
        if is_visible {
            ...
            #[cfg(target_os = "windows")]
            if observing_first_root_frame {
                log::warn!("F7 root render observation: paint completed");
            }
