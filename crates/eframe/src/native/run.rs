...
struct WinitAppWrapper<T: WinitApp> {
    ...
    deferred_direct_paint_done_this_cycle: bool,
    winit_app: T,
...
impl<T: WinitApp> WinitAppWrapper<T> {
    fn new(winit_app: T, run_and_return: bool) -> Self {
        Self {
            ...
            deferred_direct_paint_done_this_cycle: false,
            winit_app,
            return_result: Ok(()),
            run_and_return,
        }
    ...
    fn handle_event_result(
        ...
        if cfg!(target_os = "windows")
            && !self.deferred_direct_paint_in_progress
            && let Ok(EventResult::RepaintNow(window_id)) = event_result
            && (!self.deferred_direct_paint_done_this_cycle
    ...
    fn check_redraw_requests(&mut self, event_loop: &ActiveEventLoop) {
        let now = Instant::now();
        let deferred_direct_paint_allowed = cfg!(target_os = "windows")
            && !self.deferred_direct_paint_in_progress
            && !self.deferred_direct_paint_done_this_cycle;
        ...
        if !deferred_window_ids.is_empty() {
            ...
            self.deferred_direct_paint_done_this_cycle = true;
...
impl<T: WinitApp> ApplicationHandler<UserEvent> for WinitAppWrapper<T> {
    ...
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: winit::event::StartCause) {
        self.deferred_direct_paint_done_this_cycle = false;
