...
/// Controls which events are captured by the focused widget.
///
/// Captured events are handled by the widget and won't trigger global actions
/// (like focus changes). Other events are still passed to the widget but
/// remain available for global use.
#[derive(Clone, Copy, Debug)]
pub struct EventFilter {
    /// Returns true if the event should be captured by the widget.
    ///
    /// Default: `false` for navigation (Tab, Escape, Arrows), `true` for others.
...
impl EventFilter {
    /// Should this event be consumed by the widget?
